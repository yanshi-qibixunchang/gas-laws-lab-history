...
const catalogSource = readFileSync(
  join(projectRoot, 'src', 'audio', 'experiments', 'heatCapacity', 'heatCapacityAudioCatalog.ts'),
  'utf8',
);
...
const catalogFiles = [...catalogSource.matchAll(/file\('([^']+\.wav)'\)/g)].map((match) => match[1]).sort();
