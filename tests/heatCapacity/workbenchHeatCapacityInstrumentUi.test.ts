...
assert.match(workbenchSource, /progressRatio:\s*heatCapacityFreeWaitTimer\.targetS > 0[\s\S]*heatCapacityFreeWaitTimer\.elapsedS \/ heatCapacityFreeWaitTimer\.targetS/, 'Free and Guide wait timer should derive a live progress ratio from elapsed and target time');
assert.match(workbenchSource, /--studio-heat-free-wait-progress/, 'Free and Guide wait timer should pass live progress into the timer ring');
assert.match(workbenchSource, /--studio-heat-free-wait-progress-color/, 'Free and Guide wait timer should pass a live blue-to-red progress color into the timer ring');
...
assert.match(workbenchSource, /activeFile\.heatCapacityGuideWorkflow\.step === 'recordU1Required'[\s\S]*activeFile\.heatCapacityGuideWorkflow\.step === 'recordU2Required'/, 'Guide speed overlay should remain mounted at U1 and U2 record-ready checkpoints so the timer can exit only after recording');
assert.match(workbenchSource, /heatCapacityFreeSpeedOptionsDisabled[\s\S]*disabled=\{heatCapacityFreeSpeedOptionsDisabled\}/, 'Guide wait timer speed buttons should become visual-only controls after the five-minute target is reached');
assert.match(workbenchSource, /studio-heat-free-speed-shell/, 'Free and Guide wait speed overlay should render the approved integrated capsule SVG shell');
assert.match(styleSource, /\.studio-heat-free-speed-main-capsule/, 'Free and Guide wait speed overlay should style a standalone main capsule shape');
assert.match(styleSource, /\.studio-heat-free-speed-timer-cabin/, 'Free and Guide wait speed overlay should style the attached timer cabin shape');
assert.match(styleSource, /conic-gradient\(\s*var\(--studio-heat-free-wait-progress-color\)\s+0\s+var\(--studio-heat-free-wait-progress\)/, 'Free and Guide wait timer ring should fill according to live progress and color');
assert.match(styleSource, /\.studio-heat-free-speed-circle:disabled\s*\{[\s\S]*opacity:\s*1;/, 'disabled Guide wait speed buttons should keep their instrument styling while becoming non-interactive');
assert.match(styleSource, /\.studio-heat-free-speed-thumb\s*\{[\s\S]*top:\s*1px;/, 'Free and Guide wait speed active bubble should be vertically centered in the option capsule');
assert.match(styleSource, /\.studio-heat-free-speed-screw:first-of-type\s*\{[\s\S]*left:\s*7px;/, 'Free and Guide wait speed left decoration screw should be visually balanced inward from the right-side screw');
assert.match(styleSource, /\.studio-heat-free-speed-screw:last-of-type\s*\{[\s\S]*right:\s*4px;/, 'Free and Guide wait speed right decoration screw should move outward away from the x16 slot');
assert.doesNotMatch(workbenchSource, /studio-heat-free-speed-cabin-highlight/, 'Free and Guide wait timer cabin should not render an extra internal highlight line');
assert.doesNotMatch(styleSource, /\.studio-heat-free-speed-cabin-highlight/, 'Free and Guide wait timer cabin should not keep highlight-line styling');
assert.match(workbenchSource, /d="M 74 50 L 92 76 H 179 C 191 76 200 74 210 68 C 218 62 224 55 225 49 L 74 50 Z"/, 'Free and Guide wait timer cabin should terminate on the main capsule corner boundary instead of protruding past it');
assert.match(workbenchSource, /studio-heat-free-speed-overlay-guide-wait/, 'Guide wait speed overlay should have a dedicated class so it can avoid the guide checklist panel');
assert.match(styleSource, /\.studio-heat-free-speed-overlay-guide-wait\s*\{[\s\S]*margin-left:\s*-\d+px/, 'Guide wait speed overlay should shift left without changing the shared top-center slot');
...
assert.match(stateSource, /resetHeatCapacityFreeRunWorkbenchState[\s\S]*pressureZeroed:\s*false[\s\S]*pressureZeroKnobAngle:\s*0/, 'Free Mode reset should reset zeroing and zero-knob state');
...
assert.match(leftPanelSource, /const shouldShowCompletedGuideResult =[\s\S]*file\.heatCapacityMode === 'free'[\s\S]*file\.heatCapacityGuideWorkflow\.step === 'completed'[\s\S]*file\.heatCapacityGuideTrial !== null/, 'completed Guide results should remain visible after the active mode returns to Free');
assert.match(leftPanelSource, /file\.heatCapacityMode === 'free' && !shouldShowCompletedGuideResult[\s\S]*renderFreeDataAndResultsTab/, 'ordinary Free Mode panels should route to the merged Data & Results page');
...
assert.match(sceneSource, /\}\), \[cameraViewScheme, qualityProfile\]\);/, 'quality-mode canvas memoization should include the active camera scheme without remounting on theme-only changes');
...
assert.match(ultraModelSource, /const pumpPulseOwnsVisual = clock\.elapsedTime < pumpPulseVisualUntilRef\.current;[\s\S]*if \(!pumpPulseOwnsVisual && props\.pumpBulbState !== 'idle'\)/, 'Ultra GLB pump morph should not let persisted pump state create a second twitch during the active pulse');
...
assert.match(sceneSource, /focusMode === 'instrument' \|\| focusMode === 'pump'[\s\S]*cameraViewScheme\.focusViews\?\.\[focusMode\][\s\S]*const nextView = focusView \?\? \([\s\S]*cameraViewScheme\.autoDemoView \?\? cameraViewScheme\.defaultView[\s\S]*nextPosition\.set\(\.\.\.nextView\.position\)[\s\S]*nextTarget\.set\(\.\.\.nextView\.target\)/, 'camera rig should resolve remaining focus, auto-demo, and default views from the active scheme only');
...
assert.match(workbenchSource, /type ManualHeatCapacityStep =[\s\S]*openStopcockForZeroRequired[\s\S]*recordU0Required[\s\S]*recordU1Required[\s\S]*recordU2Required[\s\S]*closePowerRequired[\s\S]*completed/, 'manual heat-capacity workflow should model one guided experiment with U0/U1/U2 recording and final power-off');
...
assert.match(workbenchSource, /const recordHeatCapacityManualSample = \(kind: HeatCapacityManualRecordKind\)[\s\S]*const currentFile = filesRef\.current\.find\(\(file\) => file\.id === activeFile\.id\);[\s\S]*applyHeatCapacityGuideRecordWorkbenchState\(currentFile,\s*kind,\s*now\)/, 'direct record buttons should route through the guide domain guard using the latest active file');
assert.match(stateSource, /const activeDisplay = selectActiveHeatCapacityWorkbenchDisplay\(currentFile\);[\s\S]*displayPressureMv:\s*activeDisplay\.pressureMv[\s\S]*displayTemperatureMv:\s*activeDisplay\.temperatureMv/, 'guided U0/U1/U2 recording should write the current instrument readings into the single guide trial');
...
assert.doesNotMatch(workbenchSource, /const getActiveHeatCapacityTrial =|const isActiveTrialComplete =/, 'manual guide mode should not keep the old active-trial table helpers');
...
assert.doesNotMatch(stateSource, /const profilePressureSignalMv = profile[\s\S]*pressureSignalMv: profilePressureSignalMv \?\? file\.pressureSignalMv/, 'manual record input must not substitute profile pressure for the current instrument reading');
assert.doesNotMatch(stateSource, /const profileTemperatureSignalMv = profile[\s\S]*temperatureSignalMv: profileTemperatureSignalMv \?\? file\.temperatureSignalMv/, 'manual record input must not substitute profile temperature for the current instrument reading');
assert.match(stateSource, /recordGuideU1\(currentFile\.heatCapacityGuideTrial,\s*recordInput\)[\s\S]*recordGuideU2\(currentFile\.heatCapacityGuideTrial,\s*recordInput,\s*now/, 'manual U1 and U2 recording should preserve actual display readings in the single guide trial');
...
assert.doesNotMatch(workbenchSource, /const startNextHeatCapacityManualTrial|data-heat-capacity-mode-action="next-trial"|calculateRequired:\s*\['calculate'\]/, 'guide mode should not render or guard old next-trial/calculation actions');
...
assert.doesNotMatch(workbenchSource, /MANUAL_HEAT_CAPACITY_IDLE_HINT_DELAY_MS|MANUAL_HEAT_CAPACITY_IDLE_HINT_REPEAT_MS/, 'guide workflow should not keep obsolete 1s/3s idle hint constants after moving to ten-second strong reminders');
...
assert.match(workbenchSource, /closePumpValveRequired:\s*\['closePumpValve'\]/, 'Guide should not accept more pump-bulb presses after the displayed pressure reaches the target');
assert.doesNotMatch(workbenchSource, /closePumpValveRequired:\s*\[[^\]]*'pumpBulb'/, 'Guide close-pump-valve step must not keep the old pump-bulb action path');
...
assert.doesNotMatch(workbenchSource, /renderHeatCapacityStopcockMiniReadout|studio-heat-stopcock-mini-readout|data-heat-capacity-stopcock-mini-readout|GAUGE kPa|heatCapacityFocusMode === 'stopcock'/, 'manual stopcock mini readout should be removed with the shared valve focus mode');
...
assert.match(workbenchSource, /demoComplete:\s*'自动演示已结束，可重新开始或查看数据与结果。'/, 'fixed realtime hint should point auto-demo users to the merged Data & Results page');
...
assert.match(stateSource, /createHeatCapacityDemoTrialFromPreset/, 'auto demo completion should import the fixed preset teaching result');
assert.doesNotMatch(stateSource, /const demoTrial = createHeatCapacityDemoTrialFromProcessSamples/, 'auto demo completion should not calculate its final result from transient process samples');
assert.match(workbenchSource, /activeHeatCapacityTabId:\s*'records'[\s\S]*selectedHeatCapacityPanel:\s*'heatCapacityRecords'/, 'auto demo completion should open the merged Data & Results page');
...
assert.match(workbenchSource, /showHeatCapacityRecordSuccessSequence\(\{[\s\S]*recordMessage:\s*message[\s\S]*trialCompleteMessage:\s*kind === 'u2'/, 'successful records should display a bottom-center success toast, with a separate trial-complete toast after U2');
...
assert.match(workbenchSource, /const currentFile = filesRef\.current\.find\(\(file\) => file\.id === activeFile\.id\);[\s\S]*const attempt = applyHeatCapacityGuideRecordWorkbenchState\(currentFile,\s*kind,\s*now\);[\s\S]*updateFileById\(currentFile\.id/, 'manual U1/U2 success feedback should be driven by a synchronously calculated record result before updating React state');
...
assert.match(workbenchSource, /if \(guard\.suppressGuidance\) \{[\s\S]*if \(shouldOpenStrongReminder\) scheduleManualHeatCapacityStrongReminderAfterToast[\s\S]*return;[\s\S]*\}/, 'suppressed record-ready guard failures should keep rollback feedback, skip ordinary guidance, and still allow delayed strong reminder escalation');
...
assert.match(workbenchSource, /MANUAL_HEAT_CAPACITY_STRONG_REMINDER_DELAY_MS = 10_000/, 'guide mode strong reminder should appear after ten seconds without the target action');
...
assert.match(workbenchSource, /HEAT_CAPACITY_GUIDE_RELEASE_DURATION_MS = 350/, 'guide mode release pause should target 0.35 seconds of physical stopcock opening');
assert.match(workbenchSource, /MANUAL_HEAT_CAPACITY_GUIDANCE_PULSE_INTERVAL_MS = 4000/, 'guide mode should pulse the current target every four seconds before strong reminder escalation');
const manualPauseStepSection = workbenchSource.match(/const isManualHeatCapacityPauseStep = \(step: ManualHeatCapacityStep\) => \([\s\S]*?\);/)?.[0] ?? '';
assert.match(manualPauseStepSection, /recordU1Required[\s\S]*recordU2Required[\s\S]*closePowerRequired/, 'guide mode should pause physics at guided record and final-close checkpoints');
assert.doesNotMatch(manualPauseStepSection, /closeStopcockAfterReleaseRequired/, 'guide release-close waiting must keep physics stepping so releaseReference and particle outflow can animate');
assert.match(workbenchSource, /manualHeatCapacityActiveFileIdRef\.current === file\.id[\s\S]*isManualHeatCapacityPauseStep\(getHeatCapacityManualStep\(file\)\)[\s\S]*return file;/, 'guide mode pause steps should freeze workbench physics stepping for the active guided file');
assert.match(workbenchSource, /const shouldOpenStrongReminder = guard\.suppressStrongReminder \? false : registerManualHeatCapacityGuideMiss\(guard\);[\s\S]*showManualHeatCapacityGuidance\(guard\.expectedMessage[\s\S]*if \(shouldOpenStrongReminder\) \{[\s\S]*scheduleManualHeatCapacityStrongReminderAfterToast/, 'two wrong or premature guided clicks should show the reason toast before scheduling the strong reminder unless the current step explicitly suppresses strong escalation');
assert.match(workbenchSource, /guard\.suppressStrongReminder \? false : registerManualHeatCapacityGuideMiss\(guard\)/, 'guide waiting-step wrong clicks should be able to show ordinary feedback without accumulating strong-reminder misses');
...
assert.match(workbenchSource, /step === 'stabilizeBeforeReleaseRequired'[\s\S]*rollbackAnimation:\s*rollbackByAction\[action\][\s\S]*step === 'recoverRequired'[\s\S]*rollbackAnimation:\s*rollbackByAction\[action\]/, 'U1 and U2 five-minute waiting steps should still bounce visible controls without changing experiment state');
assert.match(workbenchSource, /manualHeatCapacityStrongReminderTimerRef\.current = window\.setTimeout\([\s\S]*activateManualHeatCapacityStrongReminder[\s\S]*MANUAL_HEAT_CAPACITY_STRONG_REMINDER_DELAY_MS/, 'guided target inactivity should escalate through the ten-second strong reminder timer');
assert.match(workbenchSource, /activeHeatCapacityManualStep === 'stabilizeBeforeReleaseRequired' \|\|[\s\S]*activeHeatCapacityManualStep === 'recoverRequired'[\s\S]*return undefined;/, 'guided five-minute waiting steps should not start the inactivity strong-reminder timer');
assert.match(workbenchSource, /manualHeatCapacityGuidancePulseTimerRef\.current = window\.setInterval\([\s\S]*pulseManualHeatCapacityControl\(guidance\.controlId\)[\s\S]*MANUAL_HEAT_CAPACITY_GUIDANCE_PULSE_INTERVAL_MS/, 'ordinary guided target hints should keep pulsing the current target before the strong mask appears');
assert.match(workbenchSource, /const heatCapacityGuideSteps = [\s\S]*getManualStepGuidance\(step\.manualStep,\s*activeFile\)\.message/, 'ordinary guided process guidance should derive the checklist rows from guide step guidance');
...
assert.match(workbenchSource, /const stepRecordKind = step\.status === 'current' && isCentered[\s\S]*data-heat-capacity-guide-step-record-action="true"[\s\S]*recordHeatCapacityManualSample\(stepRecordKind\)/, 'current guided checklist rows should own the U0/U1/U2 record button instead of a separate floating action');
...
assert.match(workbenchSource, /const activateManualHeatCapacityStrongReminder = \(controlId\?: string \| null\) => \{[\s\S]*clearManualHeatCapacityGuidance\(\);[\s\S]*pulseManualHeatCapacityControl\(controlId \?\? null\);[\s\S]*setManualHeatCapacityStrongReminderActive\(true\);/, 'activating the strong reminder should clear ordinary toasts and pulse the target without showing a duplicate center prompt');
const activateManualStrongReminderSection = workbenchSource.match(/const activateManualHeatCapacityStrongReminder = \(controlId\?: string \| null\) => \{[\s\S]*?\n  const scheduleManualHeatCapacityStrongReminderAfterToast/)?.[0] ?? '';
assert.doesNotMatch(activateManualStrongReminderSection, /showManualHeatCapacityGuidance/, 'strong reminder activation should not enqueue the ordinary guidance toast');
assert.match(workbenchSource, /const resetHeatCapacityGuideUiStateForModeChange = \(\) => \{[\s\S]*clearManualHeatCapacityStrongReminder\(\);[\s\S]*clearManualHeatCapacityGuidancePulseTimer\(\);[\s\S]*setHeatCapacityFocusResetKey\(\(key\) => key \+ 1\);[\s\S]*heatCapacityFocusSessionRef\.current = null;[\s\S]*\}/, 'guide mode exits and restarts should clear strong reminders and reset the 3D focus state');
assert.match(workbenchSource, /const exitHeatCapacityGuideMode = \(\) => \{[\s\S]*resetHeatCapacityGuideUiStateForModeChange\(\);[\s\S]*abortHeatCapacityGuideWorkbenchState/, 'stopping guide mode should abort the independent Guide runtime and return to Free mode through a fully reset guide UI state');
assert.match(workbenchSource, /const startHeatCapacityManualExperiment = \(\) => \{[\s\S]*resetHeatCapacityGuideUiStateForModeChange\(\);[\s\S]*activateHeatCapacityManualExperiment/, 'starting guide mode after a stop should reset the previous guide focus and reminder state before reinitializing the file');
assert.match(workbenchSource, /manualHeatCapacityPendingStrongReminderTimerRef/, 'wrong-click escalation should keep a separate pending-strong timer so the reason toast can fade first');
assert.match(workbenchSource, /scheduleManualHeatCapacityStrongReminderAfterToast[\s\S]*HEAT_CAPACITY_TOAST_DISPLAY_DURATION_MS[\s\S]*activateManualHeatCapacityStrongReminder/, 'second guided miss should wait for the reason toast display duration before opening the strong mask');
...
assert.doesNotMatch(workbenchSource, /nextFile\.heatCapacityMode === 'guide'[\s\S]*captureHeatCapacityWorkbenchSample\([^)]*,\s*'afterPumpSample'/, 'Guide pumping should not write old manual process samples after the independent Guide runtime is introduced');
...
assert.match(workbenchSource, /const getManualHeatCapacityMinimumU1PlatformMv = \([\s\S]*HEAT_CAPACITY_PRESSURE_WARNING_THRESHOLD_MV[\s\S]*const hasManualHeatCapacityReachedPumpTarget = \(/, 'guided pumping should require reaching the 120 mV suggested-stop target before the user can leave the pump stage');
assert.match(workbenchSource, /const pumpTargetReached = hasManualHeatCapacityReachedPumpTarget\(file\);[\s\S]*file\.pumpValveOpen && !pumpTargetReached[\s\S]*return 'pumpRequired'/, 'guide mode should keep asking for rapid pumping until the 120 mV target has been reached');
assert.match(workbenchSource, /pumpAction:\s*'请连续快速按压打气球，直到 Uₚ ≥ 120 mV。标准看 Uₚ 读数，达到后将自动退出聚焦。'/, 'Simplified Chinese guide pump instruction should tell users the exact displayed target and auto-exit behavior');
assert.match(workbenchSource, /guidePumpInputLockedRef/, 'Guide pumping should have a synchronous input lock for queued pump-click events');
assert.match(workbenchSource, /guidePumpInputLockedRef\.current[\s\S]*heatCapacityGuideWorkflow\.step !== 'pumpRequired'[\s\S]*return;/, 'queued Guide pump events should be swallowed before they can enter the state pump action');
assert.match(workbenchSource, /const guidePumpTargetReached = [\s\S]*getManualHeatCapacityDisplayedPressureMv\(nextHeatCapacityFile\)[\s\S]*HEAT_CAPACITY_PRESSURE_WARNING_THRESHOLD_MV/, 'Guide pump-target decisions should use the same one-decimal displayed pressure that the user sees');
assert.match(workbenchSource, /showManualHeatCapacityGuidance\([\s\S]*getManualStepGuidance\('closePumpValveRequired'[\s\S]*'pumpValve'[\s\S]*'manual-guide'/, 'Guide should immediately show the close-pump-valve prompt after auto-exiting pump focus');
assert.match(workbenchSource, /captureHeatCapacityWorkbenchSample\(nextFile,\s*'beforeReleaseSample'/, 'opening the stopcock for guided release should capture the release start sample');
assert.match(workbenchSource, /hasManualHeatCapacityWaitElapsed\(file,\s*'u1'\)/, 'guided U1 readiness should require the five-minute wait gate');
assert.match(workbenchSource, /hasManualHeatCapacityWaitElapsed\(file,\s*'u2'\)/, 'guided U2 readiness should require the five-minute wait gate');
assert.match(workbenchSource, /isManualHeatCapacityReleaseDurationReady\(file\)/, 'guided release completion should require the 0.35 second opening duration');
assert.match(workbenchSource, /heatCapacityToastCurrent \? \([\s\S]*data-heat-capacity-manual-step-hint="true"[\s\S]*\) : null/, 'triggered guide toasts should keep rendering in the center overlay');
...
assert.doesNotMatch(workbenchSource, /const heatCapacityGuideProcessPromptBlocked = manualHeatCapacityStrongReminderActive/, 'guided checklist should stay visible when the strong-reminder mask is active');
assert.match(workbenchSource, /const heatCapacityGuideProcessPromptBlocked = heatCapacityPressureAlarmVisible \|\|[\s\S]*null[\s\S]*getManualStepGuidance/, 'ordinary guided checklist should still yield to alarm surfaces, demo overlays, and mode notices');
...
assert.match(sceneSource, /type HeatCapacityFocusMode = 'none' \| 'instrument' \| 'pump' \| 'bottle'/, 'guide strong reminder should support a bottle-controls camera view separate from the pump focus view');
...
assert.match(sceneSource, /focusMode === 'instrument' \|\| focusMode === 'pump' \|\| focusMode === 'bottle'[\s\S]*cameraViewScheme\.focusViews\?\.\[focusMode\]/, 'camera rig should resolve instrument, pump, and bottle guide focus views from the active scheme');
assert.match(sceneSource, /props\.guideFocusMode !== undefined && props\.guideFocusMode !== null[\s\S]*setFocusMode\(props\.guideFocusMode\)[\s\S]*props\.guideFocusKey/, 'scene should apply strong-reminder guide focus when the workbench requests it');
...
assert.match(sceneSource, /window\.requestAnimationFrame\(retryProjection\)[\s\S]*emitProjectedHoles\(\)/, 'procedural guide projection should retry after mount in demand-rendered canvases instead of keeping an empty first projection');
...
assert.match(ultraModelSource, /window\.requestAnimationFrame\(retryUltraGuideProjection\)[\s\S]*emitUltraGuideTargetHoles\(\)/, 'Ultra guide projection should retry after GLB hitboxes mount so strong reminders do not fall back to a generic rectangle');
...
assert.match(workbenchSource, /manualHeatCapacityStrongReminderControlId[\s\S]*getHeatCapacityGuideStrongTargetSpec\(manualHeatCapacityStrongReminderControlId\)/, 'strong reminder rendering should use the active target control instead of a generic center ring');
...
assert.match(workbenchSource, /guideFocusKey=\{manualHeatCapacityStrongReminderFocusKey\}/, 'Workbench should pass a guide focus key so repeated strong reminders can reset the view');
...
assert.match(workbenchSource, /const clearManualHeatCapacityStrongReminderFocus = \(\) => \{[\s\S]*setHeatCapacityFocusResetKey\(\(key\) => key \+ 1\);[\s\S]*heatCapacityFocusSessionRef\.current = null;/, 'successful strong-reminder target actions should have a shared way to restore the default 3D view');
assert.match(workbenchSource, /if \(shouldResetStrongFocusAfterAllowedAction\(activeStrongReminderControlId,\s*action\)\) \{[\s\S]*clearManualHeatCapacityStrongReminderFocus\(\);[\s\S]*\}/, 'allowed strong-reminder stopcock and pump-valve actions should reset the guide camera after the user clicks the target');
assert.match(workbenchSource, /activeHeatCapacityManualStep !== 'closePumpValveRequired'[\s\S]*focusSession\?\.fileId === activeFile\.id && focusSession\.mode === 'pump'[\s\S]*exitHeatCapacityFocusMode\(\);[\s\S]*guidePumpInputLockedRef\.current = true;/, 'Guide should leave pump focus and lock further pump input after the visible pump target is reached');
...
assert.match(workbenchSource, /restoreManualHeatCapacityGuideSession[\s\S]*activateManualHeatCapacityStrongReminder\(restoredControlId \?\? guidance\.controlId\)/, 'refreshing during an active strong reminder should reopen the same target when possible and otherwise fall back to the current guide step');
assert.match(workbenchSource, /manualHeatCapacityStrongReminderActive &&[\s\S]*manualHeatCapacityStrongReminderControlId === guidance\.controlId[\s\S]*return undefined;/, 'the ordinary ten-second Guide timer should not clear an already restored strong reminder for the same step');
...
assert.match(styleSource, /\.studio-heat-manual-step-hint\s*\{[\s\S]*width:\s*min\(520px,[\s\S]*background:\s*rgba\([^)]*,\s*0\.78\)/, 'center guide feedback cards should use a consistent translucent width');
...
assert.match(leftPanelSource, /引导模式数据与结果[\s\S]*数据来源：引导模式固定标准流程/, 'recording page should rename manual mode to guide mode');
...
const freeBasicNumberParametersSection = workbenchSource.match(/const heatCapacityFreeBasicNumberParameters:[\s\S]*?\n\];/)?.[0] ?? '';
...
assert.match(freeAdvancedNumberParametersSection, /id:\s*'gamma'[\s\S]*group:\s*'gasTheory'/, 'gamma should belong to the gas-theory advanced group');
...
assert.match(workbenchSource, /riskTitle:\s*\{[\s\S]*'zh-CN':\s*'确认调整高级参数'/, 'advanced risk title should describe a confirmation step, not a later experiment-group side effect');
assert.match(workbenchSource, /riskBody:\s*\{[\s\S]*'zh-CN':\s*'高级参数会影响当前实验文件的模型判定、传感器读数和记录阈值。确认后，本实验文件后续打开高级参数不再重复提示。'/, 'advanced risk body should state current-file-first acknowledgement semantics');
assert.doesNotMatch(workbenchSource, /调整高级参数会改变后续实验组|後續實驗組|future groups/, 'advanced risk copy should not keep the old future-group wording');
...
assert.doesNotMatch(workbenchSource, /title:\s*\{\s*'zh-CN':\s*'压力信号标定'/, 'advanced parameter groups should not keep an empty pressure calibration group after pressure sensitivity moves to the basic panel');
assert.match(workbenchSource, /id:\s*'gasTheory'[\s\S]*title:\s*\{\s*'zh-CN':\s*'气体理论参数'/, 'advanced parameters should expose a semantic gas-theory group');
assert.match(workbenchSource, /id:\s*'thermalExchange'[\s\S]*title:\s*\{\s*'zh-CN':\s*'热交换模型'/, 'advanced parameters should expose a semantic thermal-exchange group');
assert.match(workbenchSource, /id:\s*'nonIdealCorrection'[\s\S]*title:\s*\{\s*'zh-CN':\s*'非理想过程修正'/, 'advanced parameters should expose a semantic non-ideal correction group');
assert.match(workbenchSource, /id:\s*'recordCriteria'[\s\S]*title:\s*\{\s*'zh-CN':\s*'记录判定与安全阈值'/, 'advanced parameters should expose a semantic record-criteria group');
assert.doesNotMatch(workbenchSource, /title:\s*\{\s*'zh-CN':\s*'气体状态模型'|title:\s*\{\s*'zh-CN':\s*'热交换与泄漏修正'|title:\s*\{\s*'zh-CN':\s*'读数采集与记录判定'/, 'advanced parameter groups should remove old merged group titles after regrouping');
