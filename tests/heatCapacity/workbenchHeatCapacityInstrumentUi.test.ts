...
assert.match(sceneSource, /const orbitControlsEnabled = focusMode === 'none' && !\(props\.cameraInteractionLocked \?\? props\.interactionLocked\);/, 'Ultra orbit controls should lock during active demo/focus while completed teaching states can still allow camera inspection');
...
assert.match(workbenchSource, /overlayBottomCenter=\{heatCapacityBottomCenterOverlay\}[\s\S]*data-heat-capacity-guide-step-hint="true"|data-heat-capacity-guide-step-hint="true"[\s\S]*overlayBottomCenter=\{heatCapacityBottomCenterOverlay\}/, 'heat-capacity guide toast queue should use one bottom-centered overlay slot');
assert.match(styleSource, /\.studio-heat-guide-step-hint \{[\s\S]*studioOverlayBottomCenterIn[\s\S]*studioOverlayBottomCenterOut/, 'bottom-centered guide hints should float in from below and fade down on exit');
...
const demoModeActionsSection = workbenchSource.match(/<div className="studio-heat-mode-actions studio-heat-mode-actions-demo"[\s\S]*?<div\s+className=\{heatCapacityModeSegmentClassName\('guide'\)\}/)?.[0] ?? '';
const demoCompletedActionBranch = demoModeActionsSection.match(/heatCapacityTeachingCompleted \? \([\s\S]*?\) : \(/)?.[0] ?? '';
assert.match(demoCompletedActionBranch, /data-heat-capacity-mode-action="exit-teaching"/, 'completed demo mode should show one explicit teaching-exit action');
assert.match(demoCompletedActionBranch, /studio-heat-mode-action-danger/, 'completed demo exit should use the same red stop-style color as completed guide exit');
assert.doesNotMatch(demoCompletedActionBranch, /data-heat-capacity-mode-action="stop-demo"|<Square/, 'completed demo mode must not keep the stop action next to the exit action');
assert.match(workbenchSource, /data-heat-capacity-mode-action=\{heatCapacityTeachingCompleted \? 'exit-teaching' : 'exit-guide'\}/, 'guide mode expansion should switch from stop to explicit exit when completed');
assert.match(workbenchSource, /heatCapacityTeachingCompleted[\s\S]{0,120}<LogOut[\s\S]{0,120}<Square/, 'guide completed exit should use a distinct exit icon while running guide still uses the stop icon');
assert.match(workbenchSource, /studio-heat-mode-action-danger[\s\S]{0,260}data-heat-capacity-mode-action=\{heatCapacityTeachingCompleted \? 'exit-teaching' : 'exit-guide'\}/, 'completed guide exit should keep the same red stop-style color');
...
assert.match(workbenchSource, /data-heat-capacity-mode-segment="guide"[\s\S]*data-heat-capacity-mode="guide"[\s\S]*data-heat-capacity-mode-action=\{heatCapacityTeachingCompleted \? 'exit-teaching' : 'exit-guide'\}[\s\S]*data-heat-capacity-mode-segment="free"/, 'guide actions should extend immediately after the guide mode segment before the free segment');
...
assert.doesNotMatch(stateSource, /const profilePressureSignalMv = profile[\s\S]*pressureSignalMv: profilePressureSignalMv \?\? file\.pressureSignalMv/, 'guide record input must not substitute profile pressure for the current instrument reading');
assert.doesNotMatch(stateSource, /const profileTemperatureSignalMv = profile[\s\S]*temperatureSignalMv: profileTemperatureSignalMv \?\? file\.temperatureSignalMv/, 'guide record input must not substitute profile temperature for the current instrument reading');
...
const guidePauseStepSection = workbenchSource.match(/const isGuideHeatCapacityPauseStep = \(step: GuideHeatCapacityStep\) => \([\s\S]*?\);/)?.[0] ?? '';
...
assert.doesNotMatch(guidePauseStepSection, /closeStopcockAfterReleaseRequired/, 'guide release-close waiting must keep physics stepping so releaseReference and particle outflow can animate');
...
assert.match(workbenchSource, /const shouldReplaceActiveGuideBlockedToast = source === 'guide-blocked';[\s\S]*showHeatCapacityToast\(message,\s*level,\s*\{ source,\s*interrupt:\s*shouldReplaceActiveGuideBlockedToast \}\)/, 'rapid repeated guide mistakes should replace the active guide-blocked toast instead of queueing stale prompts');
...
assert.match(workbenchSource, /const exitHeatCapacityGuideMode = \(\) => \{[\s\S]*resetHeatCapacityModeUiForFreeBase\(\);[\s\S]*abortHeatCapacityGuideWorkbenchState/, 'stopping guide mode should abort the independent Guide runtime and return to Free mode through the unified Free-base UI cleanup');
