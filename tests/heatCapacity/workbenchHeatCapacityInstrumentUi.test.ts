...
assert.match(sceneSource, /type CameraViewScheme = \{[\s\S]*defaultView: CameraFocusView;[\s\S]*autoDemoView\?: CameraFocusView;[\s\S]*focusViews\?: CameraFocusViews;/, 'heat-capacity camera views should be grouped into explicit model schemes');
...
assert.match(sceneSource, /const ULTRA_CAMERA_VIEW_SCHEME: CameraViewScheme = \{[\s\S]*defaultView:[\s\S]*position: \[0\.88, 3\.51, 7\.43\][\s\S]*target: \[0\.00, 0\.32, 0\.00\]/, 'Ultra GLB scheme should keep the approved model default view');
...
assert.match(sceneSource, /camera: \{ position: cameraViewScheme\.defaultView\.position, fov: 38 \}/, 'Canvas camera should use the active scheme default camera position');
...
assert.match(sceneSource, /const orbitControlsEnabled = props\.performanceMode === 'ultra'\s*\?\s*true\s*:\s*focusMode === 'none' && !props\.interactionLocked;/, 'Ultra orbit controls should remain usable while deferred mode and focus integration are being rebuilt');
...
assert.match(workbenchSource, /const HEAT_CAPACITY_HARD_SPHERE_PERFORMANCE_PRESETS = \{[\s\S]*standard:\s*\{\s*particleMultiplier:\s*1\.25,\s*speedMultiplier:\s*1\.25\s*\}[\s\S]*balanced:\s*\{\s*particleMultiplier:\s*1,\s*speedMultiplier:\s*1\s*\}[\s\S]*performance:\s*\{\s*particleMultiplier:\s*0\.5,\s*speedMultiplier:\s*0\.5\s*\}[\s\S]*ultra:\s*\{\s*particleMultiplier:\s*1\.25,\s*speedMultiplier:\s*1\.25\s*\}[\s\S]*\} as const;/, 'ultra mode should use the high-clarity hard-sphere preset now that it is the GLB model tier');
