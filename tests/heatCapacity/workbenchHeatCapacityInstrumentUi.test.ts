...
assert.match(workbenchSource, /const freeReleaseReference = activeFile\.heatCapacityFreePhysicsState\.releaseReference[\s\S]*const freeReleaseFlowActive = activeFile\.heatCapacityMode === 'free'[\s\S]*activeFile\.heatCapacityFreeStopcockFlowOpen[\s\S]*freeReleaseReference !== null[\s\S]*activeFile\.pressureDeltaKPa > 0\.08/, 'Free Mode should drive particle release from the new release reference and live pressure difference');
