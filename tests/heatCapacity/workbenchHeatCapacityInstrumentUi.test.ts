...
assert.match(workbenchSource, /const HEAT_CAPACITY_HARD_SPHERE_PERFORMANCE_PRESETS = \{[\s\S]*standard:\s*\{\s*particleMultiplier:\s*1,\s*speedMultiplier:\s*1\.25\s*\}[\s\S]*balanced:\s*\{\s*particleMultiplier:\s*0\.75,\s*speedMultiplier:\s*1\s*\}[\s\S]*performance:\s*\{\s*particleMultiplier:\s*0\.5,\s*speedMultiplier:\s*0\.5\s*\}[\s\S]*ultra:\s*\{\s*particleMultiplier:\s*1\.25,\s*speedMultiplier:\s*1\.25\s*\}[\s\S]*\} as const;/, 'skeleton hard-sphere particle counts should keep low load unchanged, move the old balanced count to high performance, and set balanced to the midpoint');
