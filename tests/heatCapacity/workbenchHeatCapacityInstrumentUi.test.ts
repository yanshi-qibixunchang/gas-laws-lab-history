...
assert.match(workbenchSource, /overlayCenterAboveGuideMask=\{activeHeatCapacityModalLocked\}/, 'modal center content should be raised above the scene interaction lock');
...
assert.match(styleSource, /\.studio-preview-overlay-center-above-guide-mask\s*\{[\s\S]*?z-index:\s*11;/, 'modal center layer should sit above the z-index 10 interaction lock');
