...
assert.match(sceneSource, /const handleScenePointerDownCapture = useCallback\(\(event: React\.PointerEvent<HTMLDivElement>\) => \{[\s\S]*event\.button === 0[\s\S]*sceneDragClickGuardRef\.current = \{[\s\S]*pointerId: event\.pointerId[\s\S]*closeValveFocusBubble\(\);[\s\S]*\}, \[closeValveFocusBubble\]\);/, 'pressing the primary pointer anywhere should arm the drag click guard and close valve hover bubbles');
