...
assert.match(workbenchSource, /const heatCapacityGuideProcessPrompt = [\s\S]*getManualStepGuidance\(activeHeatCapacityManualStep,\s*activeFile\)\.message/, 'ordinary guided process prompts should derive from the current guide step');
assert.match(workbenchSource, /const heatCapacityBottomCenterOverlay = heatCapacityGuideProcessPrompt \? \([\s\S]*data-heat-capacity-guide-process-prompt="true"/, 'ordinary guided process prompts should render in the 3D window center-lower overlay');
assert.match(styleSource, /\.studio-heat-guide-process-prompt\s*\{[\s\S]*pointer-events:\s*none;/, 'ordinary guided process prompt should be a non-interactive center-lower hint');
assert.match(styleSource, /\.studio-heat-guide-process-prompt\s*\{[\s\S]*margin-bottom:\s*min\(7vh,\s*56px\);/, 'ordinary guided process prompt should sit lower in the 3D preview to avoid covering the apparatus');
...
assert.match(workbenchSource, /manualHeatCapacityStrongReminderActive \|\| heatCapacityPressureAlarmVisible[\s\S]*null[\s\S]*getManualStepGuidance/, 'ordinary bottom guide prompts should still yield to strong reminders, alarm surfaces, demo overlays, and mode notices');
...
assert.match(workbenchSource, /activeHeatCapacityManualStep !== 'closePumpValveRequired'[\s\S]*focusSession\.mode !== 'pump'[\s\S]*exitHeatCapacityFocusMode\(\);/, 'Guide should leave pump focus after reaching the pump target so the pump valve is visible for the next action');
...
assert.match(workbenchSource, /restoreManualHeatCapacityGuideSession[\s\S]*activateManualHeatCapacityStrongReminder\(guidance\.controlId\)/, 'refreshing during an active strong reminder should reopen the strong reminder for the current guide step');
