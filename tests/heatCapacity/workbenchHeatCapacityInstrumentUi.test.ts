...
assert.match(sceneSource, /id:\s*'pressureZero'[\s\S]*ellipseScale:\s*0\.82[\s\S]*screenOffsetPx:\s*\{ y:\s*-16 \}/, 'procedural pressure-zero guide hole should be smaller and shifted farther upward from the raw hitbox projection');
...
assert.match(ultraModelSource, /id:\s*'pressureZero'[\s\S]*ellipseScale:\s*0\.82[\s\S]*screenOffsetPx:\s*\{ y:\s*-16 \}/, 'Ultra pressure-zero guide hole should use the same smaller upward visual tuning');
