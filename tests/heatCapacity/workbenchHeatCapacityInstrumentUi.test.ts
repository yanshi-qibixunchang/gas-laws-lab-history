...
assert.match(sceneSource, /const ULTRA_CAMERA_VIEW_SCHEME: CameraViewScheme = \{[\s\S]*defaultView:[\s\S]*position: \[0\.58, 3\.05, 6\.25\][\s\S]*target: \[0\.02, 0\.52, 0\.02\]/, 'Ultra GLB scheme should keep the approved closer and more frontal model default view');
...
assert.match(sceneSource, /camera: \{ position: cameraViewScheme\.defaultView\.position, fov: props\.performanceMode === 'ultra' \? 36 : 38 \}/, 'Canvas camera should use the active scheme default camera position and Ultra-specific close-view FOV');
