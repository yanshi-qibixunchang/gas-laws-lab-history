...
assert.equal(packageJson.version, '4.1.16', 'next desktop update release should bump package version to 4.1.16');

const currentRelease = findRelease('4.1.16');
...
assert.ok(currentRelease, 'release notes should include the 4.1.16 interaction and updater polish release');
for (const locale of locales) {
  assert.ok(currentRelease.summary?.[locale]?.trim(), `4.1.16 release summary should include ${locale}`);
}
assert.equal(
  currentRelease.download?.releasePage,
  'https://github.com/yanshi-qibixunchang/hard-sphere-lab-release/releases/tag/v4.1.16',
  '4.1.16 release page should be published in the public release repository',
);
assert.equal(
  currentRelease.download?.windowsInstaller,
  'https://github.com/yanshi-qibixunchang/hard-sphere-lab-release/releases/download/v4.1.16/heat-capacity-lab-setup-4.1.16.exe',
  '4.1.16 installer should be published in the public release repository',
);
...
assert.ok(
  ...
  '4.1.16 should include high-importance heat-capacity interaction updates',
);
assert.ok(
  currentItems.some((item) => item.scope === 'desktop-update' && item.importance === 'high'),
  '4.1.16 should include the structured update-notes download fix',
);
assert.ok(
  currentItems.some((item) => item.scope === 'ui' && item.importance === 'medium'),
  '4.1.16 should include the 3D focus-mode removal note',
);
