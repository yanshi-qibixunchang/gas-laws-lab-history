...
assert.equal(packageJson.version, '4.1.21', 'next desktop update release should bump package version to 4.1.21');

const currentRelease = findRelease('4.1.21');
...
assert.ok(currentRelease, 'release notes should include the 4.1.21 standard export graph clarification release');
for (const locale of locales) {
  assert.ok(currentRelease.summary?.[locale]?.trim(), `4.1.21 release summary should include ${locale}`);
}
assert.equal(
  currentRelease.download?.releasePage,
  'https://github.com/yanshi-qibixunchang/hard-sphere-lab-release/releases/tag/v4.1.21',
  '4.1.21 release page should be published in the public release repository',
);
assert.equal(
  currentRelease.download?.windowsInstaller,
  'https://github.com/yanshi-qibixunchang/hard-sphere-lab-release/releases/download/v4.1.21/heat-capacity-lab-setup-4.1.21.exe',
  '4.1.21 installer should be published in the public release repository',
);
...
assert.ok(
  ...
  '4.1.21 should include the high-importance standard export graph clarification',
);
assert.ok(
  currentItems.some((item) => item.scope === 'exporter-samples' && item.importance === 'medium'),
  '4.1.21 should include the sample payload generator alignment',
);
assert.ok(
