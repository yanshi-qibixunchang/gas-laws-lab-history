...
assert.equal(packageJson.version, '4.1.12', 'next desktop update release should bump package version to 4.1.12');

const currentRelease = findRelease('4.1.12');
...
assert.ok(currentRelease, 'release notes should include the 4.1.12 heat-capacity and Ultra GLB release');
for (const locale of locales) {
  assert.ok(currentRelease.summary?.[locale]?.trim(), `4.1.12 release summary should include ${locale}`);
}
assert.equal(
  currentRelease.download?.releasePage,
  'https://github.com/yanshi-qibixunchang/hard-sphere-lab-release/releases/tag/v4.1.12',
  '4.1.12 release page should be published in the public release repository',
);
assert.equal(
  currentRelease.download?.windowsInstaller,
  'https://github.com/yanshi-qibixunchang/hard-sphere-lab-release/releases/download/v4.1.12/heat-capacity-lab-setup-4.1.12.exe',
  '4.1.12 installer should be published in the public release repository',
);
...
assert.ok(
  ...
  '4.1.12 should include the heat-capacity Ultra GLB changes',
);
assert.ok(
  currentItems.some((item) => item.scope === 'desktop-update' && item.importance === 'medium'),
  '4.1.12 should include the auto-update distribution compatibility note',
);
