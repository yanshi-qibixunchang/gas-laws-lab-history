...
> 格式按 GB/T 7714 风格整理。网络资料访问日期按实际核验日期记录；项目映射口径更新于 2026-07-13。每条后附“本项目用途”，说明它在模型设计中对应哪一部分。
...
- [3] 支撑验收操作组：5min 等待、`18` 次打气首末跨度 `8s`、快速开闭及开口时间变化都应进入测试矩阵；项目统一标准点为 `0.375s`，最佳时序窗口为 `0.30–0.50s`。
