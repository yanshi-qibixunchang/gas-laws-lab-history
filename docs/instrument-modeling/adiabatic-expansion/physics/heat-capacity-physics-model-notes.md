...
releaseOptimalMinS = 0.50
releaseOptimalMaxS = 0.70
autoDemoReleaseDurationS = 0.60
...
- 标准指导场景的物理气流和“咻”声约在主放气开始后 `0.33 s` 达到压差平衡阈值；不得为了匹配评分窗口而调整流量。
- `0.60 s` 是包含用户反应时间的标准关阀代表点，应接近理论 gamma；明显过短和过长的真实操作应产生更大的绝对误差。
- 自动演示固定使用 `0.60 s`；自由模式允许用户随时关闭，`0.50–0.70 s`（含边界）获得完整时序分；指导模式在物理压差达到平衡阈值时冻结，等待用户完成关闭操作。用户在形成真实气流后主动提前关闭时，应接受操作并在关闭动画完成后直接进入 U2 等待；未形成气流的快速开关仍留在放气步骤重试。
...
- 连续放气只在 `420 ms` 开启动画完成后开始；自动演示主放气使用 `0.60 s` 标准代表点。
