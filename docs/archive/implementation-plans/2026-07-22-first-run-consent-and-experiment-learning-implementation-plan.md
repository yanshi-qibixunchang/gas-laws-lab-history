...
> 状态：第一、第二小版本已通过用户体验验收，第三小版本已实现并通过全量自动验收，待用户体验验收
>
> 制定日期：2026-07-22
>
> 总体设计：`docs/archive/historical-specs/2026-07-22-first-run-consent-and-experiment-learning-design.md`
...
- [x] 在渲染 `WorkbenchStudioPrototype` 前判断入口分支。
- [x] 保留现有 IndexedDB 初始化和安全模式优先级。
- [x] 首次档案缺失时进入完整流程。
- [x] 仅许可版本变化时进入许可单独确认。
- [x] 许可完成后恢复教程或普通工作台。
- [x] 首次流程中的语言和问题答案只存内存草稿。
...
- [x] 支持简体中文、繁体中文和 English。
- [x] 全新用户按 `navigator.languages`、浏览器环境和桌面系统语言匹配。
- [x] 无匹配时使用简体中文。
- [x] 旧版语言设置存在时优先使用旧设置。
- [x] 选择后立即更新首次流程文案，但不提前持久化。
- [x] 许可同意后再同步到现有通用设置。
...
- [x] 5 秒循环轮播。
- [x] 手动切换后重置计时。
- [x] 悬停、焦点进入、页面隐藏和最小化时暂停。
- [x] 返回语言再前进时不重复欢迎动画。
- [x] 通过 `matchMedia('(prefers-reduced-motion: reduce)')` 检测系统减少动态效果；匹配时停止轮播并缩短动画。
- [x] 卡片页始终允许直接点击“下一步”。
...
- [x] 架构保留三个问题，当前只渲染绝热膨胀问题。
- [x] 标题使用“您是否了解本软件绝热膨胀法实验的操作逻辑？”。
- [x] 辅助说明使用“请根据您是否能够使用本软件完成模式选择、仪器操作、数据记录与结果处理进行选择。”。
- [x] 答案使用“是，我已了解”和“否，我需要引导”。
- [x] 默认不选择，未选择时禁用“下一步”。
- [x] 返回卡片后保留答案。
- [x] 同意许可后，“已了解”直接设为完全解锁；“需要引导”立即激活教程。
...
- [x] 主文档滚动容器和二级许可证材料复用现有内容。
- [x] 15 秒与滚动到底两个条件独立。
- [x] 页面隐藏或最小化时暂停累计。
- [x] 内部二级材料不暂停、不重置。
- [x] 固定底栏显示两个完成条件。
- [x] 同意按钮在两个条件完成前不可点击。
- [x] 主按钮固定为“我已阅读并知悉”，次要按钮固定为“不同意并退出”。
- [x] 无窗口关闭按钮、无返回、无 Esc、无遮罩关闭。
- [x] “不同意并退出”调用桌面安全退出或网页阻断。
- [x] “关于”中的普通许可查看保持现有可关闭逻辑，不受强制条件影响。

### 7.7 任务 6：提交与许可版本变化

- [x] 完整首次流程同意后，一次性提交首次完成状态、许可版本、语言、问题答案和学习里程碑。
- [x] “需要引导”提交成功后立即执行第二版教程协调器。
- [x] 用户拒绝或从系统标题栏退出时不写入草稿。
- [x] 许可版本变化只显示强制许可窗口。
- [x] 许可版本变化且教程未完成时，先同意许可再重建教程。
- [x] 增加人工许可版本与内容修订检查。

### 7.8 任务 7：补齐设置入口

- [x] 开放“重新观看产品介绍”，只播放欢迎和卡片。
- [x] 正式模式运行时暂停，结束后恢复。
- [x] 开放“重新选择学习需求”，只显示当前已开放问题。
- [x] 从已了解改为需要引导时显示文件暂时锁定的后果确认。
- [x] 保留第二版的直接重置入口。
- [x] 教程进行期间三个入口全部禁用。
