...
**2026-06-19 调整：** 推进顺序改为“先完成全部模型优化，再统一调参和六类验收，最后再适配 UI”。UI 待适配和 UI 待添加清单仍保留，但在三个新增模型完成前不作为执行入口。三模型详细计划现归档于 `docs/archive/implementation-plans/2026-06-19-heat-capacity-free-mode-model-first-optimization.md`。
...
- [x] 当前验收口径已切换到 `docs/instrument-modeling/heat-capacity-free-mode-six-class-validation-plan.md` 的六类测试方案。
- [x] 当前 Free Mode 没有独立环境扰动模型；本轮“完全真实”测试只包含热交换、漏气、传感器滞后、量化和传感器噪声，环境扰动仍属于模型待添加。
...
> 当前状态：整体后置。根据 2026-06-19 的新决策，以下 UI 项只作为待办清单保存，必须等现有模型链路、三个新增模型、统一调参和六类验收完成后再执行。
...
> 当前状态：整体后置。新增 UI 必须等三模型和代码层真实/理想参数组稳定后再做。
...
- [ ] 完成所有 P0 模型待适配项。
- [ ] 确认当前连续泵气、放气、热交换、封闭漏气和传感器链路都有聚焦测试。
- [ ] 运行：
...
- [ ] 新增传感器低压不敏感模型，用于处理打气过少时低信号 gamma 仍看似正常的问题。
- [ ] 新增打气阀打开时泄漏/泵腔交换模型，用于让慢打气通过真实物理过程影响结果。
- [ ] 新增独立环境扰动模型，用于真实模式的环境温度/压力确定性漂移。
- [ ] 每个新增模型都必须先有独立领域测试，再接入总物理链路。

详细执行计划见：

```text
docs/archive/implementation-plans/2026-06-19-heat-capacity-free-mode-model-first-optimization.md
```

### 阶段 3：统一参数组和六类验收

- [ ] 建立代码层理想参数组：关闭低压非线性、泵阀泄漏/泵腔交换、环境扰动、漏气和噪声。
- [ ] 建立代码层真实默认参数组：开启热交换、弱漏气、传感器滞后/量化/噪声、低压不敏感、泵阀泄漏/泵腔交换和弱环境扰动。
- [ ] 用六类测试重新扫描绝对理想、理想实验、最佳真实、合适真实、单项扫描和单项错误。
- [ ] 更新基线结果文档。

### 阶段 4：建立诊断数据和评分基础

- [ ] 新增实验过程诊断数据层。
- [ ] 用过程数据解释打气不足、打气过多、慢打气、放气过短、放气过久、U1/U2 等待错误和漏气敏感等待。
- [ ] 基于过程诊断数据重建阶段判断。
- [ ] 更新评分报告，让它能指出主导错误操作。

### 阶段 5：最后适配 UI

- [ ] 适配参数调整窗口的标签、分组和真实/理想模式切换。
- [ ] 适配计时器、倍速条和等待流程显示。
- [ ] 适配完整诊断报告 UI。
- [ ] 适配预警/报警视觉等级和交互。
- [ ] 检查固定预览：
...
- [ ] 按模型优先计划先完成阶段 1：
  - 当前连续泵气过程稳定测试。
  - 当前六类测试基线确认。
  - `stopcockFlowRate`、`gamma`、热交换、封闭漏气、倍速等价性覆盖确认。
