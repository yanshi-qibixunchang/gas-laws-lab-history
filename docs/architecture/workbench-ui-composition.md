...
本文件固定 ARCH-003 完成后的职责边界。2026-09-06 用户将交付断点调整到整体架构拆分完成，内部检查点不作为整体交付。业务、状态资源与生命周期已迁入明确所有者；主入口保留显式页面装配、命名端口连接和轻量显示投影。版本保持 6.4.0，不推送、不发布。全局检查、5174 浏览器验收和性能证据集中记录在[当前交接第 4.15 节](../current-development-handoff.md)。
...
- 物理步进、主曲线、采样频率、历史容量、文件持久化格式及版本号保持不变。
