...
6. 真实/理想参数域与 `heatCapacityFreeRunWorkspace`、`heatCapacityFreeInstrumentConfig` 共同构成物理引擎所需的当前现场投影；运行工作区保存批次、轨迹、试次和活动尝试，仪器配置对象保存低频有效配置，两者都不得反向覆盖实验组集合。受控操作先更新当前现场，再由统一适配路径提交到当前可执行实验组；保存、恢复和历史查看则从实验组与参数域重新投影当前现场。
