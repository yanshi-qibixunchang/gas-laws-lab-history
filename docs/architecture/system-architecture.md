...
| 工作台状态适配 | `workbenchHeatCapacityStateTypes.ts`、`workbenchHeatCapacityInstrumentState.ts`、`workbenchHeatCapacityFreeParameterState.ts`、`workbenchHeatCapacityFreeTrialState.ts`、`workbenchHeatCapacityFreeExperimentGroupState.ts`、`workbenchHeatCapacityRuntimeDefaults.ts`、`workbenchHeatCapacityFreeRuntimeState.ts`、`workbenchHeatCapacityFreeAttemptState.ts`、`workbenchHeatCapacityFreeTraceState.ts`、`workbenchHeatCapacityFreeRecordState.ts`、`workbenchHeatCapacityFreeRollbackState.ts`、`workbenchHeatCapacityFreeRuntimeCoordinator.ts`、`workbenchHeatCapacityFreeControlState.ts`、`workbenchHeatCapacityDisplayState.ts`、`workbenchHeatCapacityFreeRunReset.ts`、`workbenchHeatCapacityCalculationCoordinator.ts`、`src/features/workbench/workbenchState.ts` 及同目录绝热膨胀专用模块 | 定义绝热膨胀状态类型边界、Free 运行/配置/高频仪器状态工作区、参数编辑与冻结、当前试次选择、实验组配置/方案切换/历史查看、尝试生命周期、轨迹证据、记录与重录、回滚、物理步进、仪器操作、显示源、完整重置、跨模式计算和纯仪器计算；为旧调用方维持兼容转发入口 | 复制已经存在于领域对象中的权威状态，让新模块反向依赖兼容入口，或绕过证据链直接修改记录、回滚与评分输入 |
