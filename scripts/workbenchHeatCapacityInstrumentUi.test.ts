...
assert.match(stateSource, /resetHeatCapacityFreeRunWorkbenchState[\s\S]*powerOn:\s*false[\s\S]*stopcockAngleDeg:\s*HEAT_CAPACITY_STOPCOCK_CLOSED_ANGLE_DEG[\s\S]*pumpValveOpen:\s*false[\s\S]*heatCapacityFreeTrials:\s*\[\]/, 'Free Mode reset should clear the current run and return apparatus controls to their initial state');
