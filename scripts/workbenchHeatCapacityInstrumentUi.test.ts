...
assert.match(workbenchSource, /data-heat-capacity-live-charts="true"/, 'fixed realtime window should keep the live chart area');
...
assert.match(workbenchSource, /label: `阶段：\$\{phaseLabel\}`/, 'fixed realtime header should keep stage as the only always-visible badge');
assert.match(workbenchSource, /phase === 'demoComplete'[\s\S]*return '演示结束'/, 'fixed realtime header should label the final auto-demo state as a demo ending, not formal experiment completion');
...
assert.match(workbenchSource, /自动演示已结束，可重新开始或查看后续数据处理结果。/, 'fixed realtime hint should keep the demoComplete copy in the auto-demo context');
assert.match(workbenchSource, /if \(activeFile\.heatCapacityPhase === 'demoComplete'\) return '自动演示已结束，可重新开始或查看后续数据处理结果。';[\s\S]*if \(!activeFile\.powerOn/, 'fixed realtime hint should check demoComplete before the powered-off fallback');
assert.match(workbenchSource, /autoDemoPaused\s*\?\s*'自动演示暂停'[\s\S]*autoDemoRunning\s*\?\s*'自动演示'[\s\S]*'自动演示准备'/, 'fixed realtime header should show automation badges only for active demo states');
assert.match(workbenchSource, /label: '操作锁定'/, 'fixed realtime header should show lock status only when interaction is locked');
...
assert.match(workbenchSource, /ΔP \/ kPa/, 'current pressure-difference reading should use the delta symbol and SI casing');
assert.doesNotMatch(workbenchSource, /Delta P \/ kPa/, 'current pressure-difference reading should not use the spelled-out Delta label');
...
assert.match(workbenchSource, /const history = activeFile\.powerOn \? activeFile\.heatCapacityTrace\.slice\(-80\) : \[\]/, 'heat capacity charts should not draw stored traces while powered off');
assert.match(workbenchSource, /history\.length < 2/, 'heat capacity charts should show an empty or collecting state until enough live points exist');
assert.doesNotMatch(workbenchSource, /getHeatCapacityChartDisplayNoise|const chartValue = trendValue \+ displayNoise/, 'heat capacity chart renderer should not create a second chart-only noise layer');
assert.match(workbenchSource, /getHeatCapacityTracePath/, 'heat capacity trace charts should render a smoothed monitoring trace path');
assert.match(workbenchSource, /getHeatCapacityTraceTitle/, 'heat capacity trace titles should adapt to the active UI language');
assert.match(workbenchSource, /压强差信号 U_p/, 'pressure chart should prefer a Simplified Chinese title in zh-CN mode');
assert.match(workbenchSource, /温度信号 U_T/, 'temperature chart should prefer a Simplified Chinese title in zh-CN mode');
assert.match(workbenchSource, /壓強差信號 U_p/, 'pressure chart should provide a Traditional Chinese title in zh-TW mode');
assert.match(workbenchSource, /Pressure Signal U_p/, 'pressure chart should keep an English title fallback');
...
assert.match(workbenchSource, /const chartLeft = 12/, 'heat capacity trace plot should keep the terminal dot away from the left edge');
assert.match(workbenchSource, /const chartWidth = 336/, 'heat capacity trace plot should reserve right-side padding for the terminal dot');
assert.match(workbenchSource, /studio-heat-live-pill/, 'heat capacity trace charts should include a restrained LIVE indicator');
assert.match(workbenchSource, /studio-heat-trace-terminal-dot/, 'heat capacity trace charts should show a terminal live dot');
assert.match(workbenchSource, /studio-heat-trace-zero-line/, 'heat capacity pressure trace should support a zero reference line for small release rebound');
assert.doesNotMatch(workbenchSource, /Math\.random/, 'chart display disturbance should be deterministic and must not create random experiment data');
...
assert.match(styleSource, /\.studio-heat-trace-terminal-dot/, 'heat capacity trace terminal dot should be styled as a live endpoint');
