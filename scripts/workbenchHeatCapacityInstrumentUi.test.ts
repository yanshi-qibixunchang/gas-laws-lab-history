...
assert.doesNotMatch(workbenchSource, /const modeLabel = [^\n]*'鎵嬪姩'/, 'fixed realtime header should not keep a default manual-mode badge');
...
assert.doesNotMatch(workbenchSource, /<div><span>鍘嬪己浣欓噺<\/span>/, 'fixed realtime window should not show pressure headroom as a constant debug field');
assert.doesNotMatch(workbenchSource, /<div><span>鍘嬪姏瀹夊叏<\/span>/, 'operation status should not repeat pressure safety under the safety card');
...
assert.doesNotMatch(workbenchSource, /鎵撴皵杩囧揩|tooFast|棰戠巼鍋忛珮/, 'workbench should not show or calculate a too-fast pump state');
