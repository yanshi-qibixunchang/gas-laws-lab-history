    ...
    async (targetZoom: number, targetUrl: string, token: number) => {
      ...
      try {
        ...
        for (let pageNum = 1; pageNum <= pdf.numPages; pageNum += 1) {
          ...
          wrapper.className =
            'w-full overflow-hidden rounded-panel bg-white shadow-sm dark:bg-slate-900/60';
  ...
  const modal = (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm">
      <button
        ...
        aria-label="Close"
      />
      <div className="pdf-modal relative z-[201] flex h-full w-full flex-col bg-white dark:bg-slate-900 md:h-[92vh] md:w-[92vw] md:rounded-panel md:border md:border-slate-200 md:shadow-2xl dark:md:border-slate-800">
        ...
        >
          {loadError && (
            <div className="mb-6 rounded-panel border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 dark:border-amber-900 dark:bg-amber-900/30 dark:text-amber-300">
              {loadError}
            </div>
          )}
