...
export const recordHeatCapacityU1 = (
  ...
  if (!trial || index < 0) {
    return { ok: false, message: '没有可记录的实验组。', trials, nextActiveTrialIndex: 0 };
  }
  if (!input.powerOn) {
    return { ok: false, message: '请先打开电源。', trials, nextActiveTrialIndex: index };
  }
  if (!canRecordU1InPhase(input.phase)) {
    return { ok: false, message: '当前阶段不能记录 U1 / UT1。', trials, nextActiveTrialIndex: index };
  }
  ...
  if (input.pressureSignalMv < MIN_U1_SIGNAL_MV) {
    return { ok: false, message: 'U_p 过小，尚未形成有效加压状态。', trials, nextActiveTrialIndex: index };
  }
...
export const recordHeatCapacityU2 = (
  ...
  if (!canRecordU2InPhase(input.phase)) {
    return { ok: false, message: '当前阶段不能记录 U2 / UT2。', trials, nextActiveTrialIndex: index };
  }
  if (trial.U1Mv === null || trial.UT1Mv === null) {
    return { ok: false, message: '请先记录当前组的 U1 / UT1。', trials, nextActiveTrialIndex: index };
  }
  if (!isFiniteNumber(input.pressureSignalMv) || !isFiniteNumber(input.temperatureSignalMv)) {
    return { ok: false, message: '当前仪表读数无效，无法记录。', trials, nextActiveTrialIndex: index };
  }
  if (input.pressureSignalMv >= trial.U1Mv) {
    return { ok: false, message: 'U2 应小于 U1，请等待回温稳定后再记录。', trials, nextActiveTrialIndex: index };
  }
  if (input.pressureSignalMv < MIN_RECOVERY_U2_SIGNAL_MV) {
    return { ok: false, message: 'U2 过低，疑似放气瞬间读数，请等待回温。', trials, nextActiveTrialIndex: index };
  }
