...
export const createHeatCapacityExperimentProfile = (
  ...
  const initialTemperatureMv = randomUniform(random, 1500.2, 1501.8);
  const stableTemperatureMv = initialTemperatureMv + randomUniform(random, 20, 29);
  const releaseTemperatureLowMv = initialTemperatureMv - randomUniform(random, 6, 10);
  const recoveryTemperatureMv = initialTemperatureMv + randomUniform(random, 0.2, 1.8);
