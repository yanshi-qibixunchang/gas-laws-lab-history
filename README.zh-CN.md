...
当前已公开发布的桌面稳定版是 `v5.1.2`。`main` 可以包含该标签之后已经评审的开发内容；只有在明确确定版本号、同步更新版本文件并发布完整更新资产后，主线源码才构成新的桌面版本。
...
- `v5.1.2` 严格恢复受影响的 4.2.3 工作区，在只读持久化故障页中加入更新自救，完善 WebGL 上下文丢失恢复，并启用三语言“气律实验室”品牌，同时保持既有用户资料目录和更新身份不变。
