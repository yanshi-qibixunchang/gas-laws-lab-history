...
当前已公开发布的桌面稳定版是 `v6.2.1`。`main` 可以包含该标签之后已经评审的开发内容；只有在明确确定版本号、同步更新版本文件并发布完整更新资产后，主线源码才构成新的桌面版本。
...
- `v6.2.1` 是活塞振动法稳定性补丁：修复自由模式第一次采集和周期框选结果进入生产界面时的崩溃，安全恢复旧版本保存的框选数据，并取消没有物理依据的 `2 m/s` 回弹速度上限。
- `v6.2.1` 同时恢复松开螺钉后的可见下沉，静默处理真实的运动执行故障，重启时清除按压、下落、回弹等瞬时状态但保留可靠的自由模式数据，并增加局部和外层显示保护且继续复用真实桌面窗口控制。
