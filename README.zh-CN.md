...
当前已公开发布的桌面稳定版是 `v5.1.1`。`main` 可以包含该标签之后已经评审的开发内容；只有在明确确定版本号、同步更新版本文件并发布完整更新资产后，主线源码才构成新的桌面版本。
...
- `v5.1.1` 可以在重新载入后恢复热容比实验的模式会话、时间线、暂停状态、精确视角、已打开窗口、引导/提醒状态和 3D 最后一帧，并保持严格的旧版迁移与运行时恢复边界。
