...
当前已公开发布的桌面稳定版是 `v6.3.0`。`main` 可以包含该标签之后已经评审的开发内容；只有在明确确定版本号、同步更新版本文件并发布完整更新资产后，主线源码才构成新的桌面版本。
...
- `v6.3.0` 为活塞振动法自由模式补齐连续热过程、压力传感、按压与虚拟双手采集链。正式记录会跟随实际按压和松手过程，同时保留既有实验计划、周期处理、拟合与计算流程。
- `v6.3.0` 为活塞实验增加电源、软管、锁紧螺钉、活塞振动和触底碰撞音效，并让适用模式中的高度复位按实际落差给出触底反馈。
- `v6.3.0` 集中修复引导流程：迁移旧存档中的隐藏基线步骤，避免提醒后正确操作仍被拒绝；记录到 `0.500 s` 后先冻结曲线并开放暂停推进；参数输入支持一致的回车确认；旋钮步骤增加可容忍的继续旋转、清晰方向箭头，以及分级的反向操作提示。演示模式中的调高与软管动作耗时同时缩短 50%。
