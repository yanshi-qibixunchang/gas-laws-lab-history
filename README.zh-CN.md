...
- 修复安装包内导出器缺少 Matplotlib PDF 后端的问题，安装后的程序可直接导出 PDF 报告和 PDF/PNG 图像。
- 将应用版本、包元数据、安装器元数据和文档统一更新到 `4.1.2`。
- 修复 Windows 安装器产品名、快捷方式名和卸载提示中的乱码。
- 通过非破坏性依赖更新降低依赖安全风险。
- 收紧工作台状态克隆、导出数据、热容比运行时状态和旧布局迁移的 TypeScript 类型。
- 补充中英文 README，覆盖仓库结构、Web 部署、桌面打包、界面区域、使用流程和导出细节。
